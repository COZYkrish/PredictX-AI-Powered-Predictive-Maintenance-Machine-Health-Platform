...
export default function AnalyticsPage() {
  ...
  if (!selectedDeviceId) {
    return (
      <div className="relative min-h-[70vh] flex flex-col items-center justify-center space-y-4 text-center font-mono">
        {/* FaultyTerminal Ambient Background */}
        <div className="fixed inset-0 pointer-events-auto z-0 overflow-hidden opacity-85">
          <FaultyTerminal
            scale={1.4}
            gridMul={[2, 1]}
            digitSize={1.2}
        ...
        <div className="relative z-10 bg-black/60 backdrop-blur-2xl border border-white/20 p-8 rounded-3xl shadow-2xl max-w-md">
          ...
          <p className="text-xs text-white/70 mt-1">Select a device from the top navigation to view historical analytics.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="relative min-h-screen">
      ...
      {/* 🌟 FaultyTerminal Ambient Background from React Bits */}
      <div className="fixed inset-0 pointer-events-auto z-0 overflow-hidden opacity-85">
        <FaultyTerminal
          scale={1.4}
          gridMul={[2, 1]}
      ...
      <div className="relative z-10 max-w-7xl mx-auto space-y-6 font-manrope">
        ...
        {hasPredictions && (
          <div className="grid gap-4 md:grid-cols-4 font-mono">
            <div className="bg-black/60 backdrop-blur-xl border border-white/20 rounded-2xl p-5 shadow-2xl">
              <span className="text-[10px] text-white/60 font-bold uppercase tracking-widest block mb-1">ACTIVE ML ENGINE</span>
              <div className="text-xl font-black text-white">{latestPrediction.model_name || 'XGBoost'}</div>
              <p className="text-[10px] text-white/50 mt-1 uppercase">v{latestPrediction.model_version || '1.0.0'}</p>
            </div>
            <div className="bg-black/60 backdrop-blur-xl border border-white/20 rounded-2xl p-5 shadow-2xl">
              <span className="text-[10px] text-white/60 font-bold uppercase tracking-widest block mb-1">INFERENCE LATENCY</span>
              <div className="text-xl font-black text-white">{latestPrediction.inference_duration_ms || 0} ms</div>
              <p className="text-[10px] text-white/50 mt-1 uppercase">Real-time inference cycle</p>
            </div>
            <div className="bg-black/60 backdrop-blur-xl border border-white/20 rounded-2xl p-5 shadow-2xl">
              <span className="text-[10px] text-white/60 font-bold uppercase tracking-widest block mb-1">EVALUATIONS</span>
              <div className="text-xl font-black text-white">{predictions.length}</div>
              <p className="text-[10px] text-white/50 mt-1 uppercase">Logged window</p>
            </div>
            <div className="bg-black/60 backdrop-blur-xl border border-white/20 rounded-2xl p-5 shadow-2xl">
              <span className="text-[10px] text-white/60 font-bold uppercase tracking-widest block mb-1">LAST INFERENCE</span>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {/* Telemetry Chart */}
          <div className="md:col-span-2 bg-black/60 backdrop-blur-xl border border-white/20 rounded-2xl p-6 shadow-2xl space-y-4">
            <div>
              <h3 className="text-xs font-mono font-bold uppercase tracking-wider text-white">CPU & MEMORY HISTORICAL OCCUPANCY</h3>
              <span className="text-[10px] font-mono text-white/50 uppercase">Rolling telemetry snapshots</span>
            </div>
            <div>
              {loadingTelemetry ? (
                ...
                <div className="flex h-[280px] items-center justify-center text-white/50 font-mono text-xs">
          ...
          <div className="bg-black/60 backdrop-blur-xl border border-white/20 rounded-2xl p-6 shadow-2xl space-y-4">
            <div>
              <h3 className="text-xs font-mono font-bold uppercase tracking-wider text-white">RISK DISTRIBUTION</h3>
              <span className="text-[10px] font-mono text-white/50 uppercase">Classified telemetry intervals</span>
            </div>
            <div>
              {loadingPredictions ? (
                <div className="h-[280px] w-full animate-pulse bg-white/5 rounded-xl" />
        ...
        {mlStatus && (
          <div className="space-y-4 font-mono">
            <div className="flex items-center gap-3">
              ...
              <span className="bg-white/10 text-white text-[10px] font-bold px-2 py-0.5 rounded-sm border border-white/20">
              ...
              return (
                <div className="grid gap-4 md:grid-cols-4">
                  {[{label: 'F1 SCORE', value: fmt(mx.f1), desc: 'Harmonic mean of precision & recall'},
                    ...
                    <div key={item.label} className="bg-black/60 backdrop-blur-xl border border-white/20 rounded-2xl p-5 shadow-2xl">
                      <span className="text-[10px] text-white/60 font-bold uppercase tracking-widest block mb-1">{item.label}</span>
                      <div className="text-2xl font-black text-white">{item.value}</div>
                      <p className="text-[10px] text-white/50 mt-1 uppercase">{item.desc}</p>
                    </div>
                  ))}
                </div>
              );
            ...
            <div className="grid gap-6 md:grid-cols-2">
              ...
              <div className="bg-black/60 backdrop-blur-xl border border-white/20 rounded-2xl p-6 shadow-2xl space-y-4">
                <div>
                  ...
                  <span className="text-[10px] text-white/50 uppercase">5 candidate models evaluated</span>
                </div>
                  ...
                  {mlStatus.model_comparison?.map((m: any) => {
                    ...
                    return (
                      <div key={m.model_name} className={`grid grid-cols-5 gap-2 text-xs py-2 px-2 rounded-lg items-center ${
                        m.is_active ? 'bg-white/15 text-white font-bold' : 'text-white/70 hover:bg-white/5'
                      }`}>
              ...
              {mlStatus.feature_importance?.length > 0 && (
                <div className="bg-black/60 backdrop-blur-xl border border-white/20 rounded-2xl p-6 shadow-2xl space-y-4">
                  <div>
                    ...
                    <span className="text-[10px] text-white/50 uppercase">XGBoost temporal feature weights</span>
                  </div>
                  <div className="h-[220px]">
                    <ResponsiveContainer width="100%" height="100%">
                      ...
                      >
                        ...
                        <XAxis type="number" domain={[0, 100]} tickFormatter={v => `${v}%`} fontSize={10} stroke="#888888" />
                        <YAxis type="category" dataKey="feature" width={140} fontSize={10} stroke="#888888" />
        ...
        <div className="bg-black/60 backdrop-blur-xl border border-white/20 rounded-2xl p-6 shadow-2xl font-mono">
          ...
          <div className="overflow-x-auto">
            {loadingPredictions ? (
              <div className="h-32 flex items-center justify-center text-white/50 text-xs">Querying inference logs...</div>
            ) : hasPredictions ? (
              <table className="w-full text-left text-xs">
                <thead className="bg-black/50 text-white/50 text-[10px] uppercase border-b border-white/10">
                  <tr>
              ...
              <div className="h-32 flex items-center justify-center text-white/50 text-xs">
