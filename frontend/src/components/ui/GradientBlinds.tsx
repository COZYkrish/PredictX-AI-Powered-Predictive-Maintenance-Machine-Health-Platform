...
export interface GradientBlindsProps extends React.HTMLAttributes<HTMLDivElement> {
  className?: string;
  dpr?: number;
  paused?: boolean;
...
export const GradientBlinds: React.FC<GradientBlindsProps> = ({
  ...
  dpr,
  paused = false,
  gradientColors = ['#1a1a1a', '#ff3000', '#0a0a0a', '#ffffff'],
  angle = 24,
  noise = 0.3,
  blindCount = 16,
  blindMinWidth = 60,
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('WebGL not supported for GradientBlinds', e);
      return;
    }
...
`;
  ...
  return (
    <div
      ref={containerRef}
      className={cn('gradient-blinds-container', className)}
      style={{
