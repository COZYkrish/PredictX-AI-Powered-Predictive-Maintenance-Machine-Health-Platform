...
export default function Chapter04Crucible() {
  ...
  return (
    <section
      id="chapter-04"
      className="relative border-b-4 border-black bg-[#F8F8F8] h-screen min-h-screen w-full flex items-center justify-center px-6 md:px-12 lg:px-16 pt-20 pb-8 swiss-diagonal overflow-hidden"
    >
      <div className="max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-14 items-center">
        ...
        <div className="lg:col-span-7 text-left">
          ...
          <ScrollReveal delay={320} direction="up" distance={12}>
            <div className="mt-5 p-4 bg-white border-2 border-black font-mono text-xs text-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
              ...
              <p className="text-black/75 uppercase text-xs">
        ...
        <ScrollReveal delay={180} direction="left" distance={30} className="lg:col-span-5 w-full">
          <div className="border-4 border-black bg-white flex flex-col shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] transition-transform duration-300 hover:-translate-y-1">
            ...
            <div className="bg-swiss-red text-white px-5 py-3.5 flex justify-between items-center border-b-4 border-black">
              ...
              <span className="bg-black text-white font-mono text-xs font-bold px-2.5 py-1 uppercase">
                {inc.severity}
              </span>
            </div>
            ...
            <div className="grid grid-cols-3 border-b-2 border-black bg-black">
              <div className="bg-white p-3 text-center border-r-2 border-black">
                ...
                <span className="text-xl sm:text-2xl md:text-3xl font-black font-mono text-swiss-red">{inc.observed}%</span>
              </div>
              <div className="bg-white p-3 text-center border-r-2 border-black">
                <span className="text-[9px] font-bold text-black/50 uppercase block font-mono">DELTA</span>
                <span className="text-xl sm:text-2xl md:text-3xl font-black font-mono text-black">{inc.deltaFromNormal}</span>
              </div>
              <div className="bg-white p-3 text-center">
                <span className="text-[9px] font-bold text-black/50 uppercase block font-mono">DURATION</span>
                <span className="text-xl sm:text-2xl md:text-3xl font-black font-mono text-black">{inc.duration}</span>
              </div>
            </div>
                ...
                {inc.likelyContributors.map((c) => (
                  <div key={c.rank} className="flex justify-between items-center bg-white p-2 border border-black/20">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-bold text-black text-xs sm:text-sm uppercase truncate max-w-[200px]">{c.process}</span>
                    </div>
                    <div className="flex items-center gap-2">
                ...
                {inc.remediationProtocol.map((step, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={`text-[11px] sm:text-xs uppercase font-bold ${isChecked ? 'line-through opacity-80' : ''}`}>
                        {step}
                      </span>
                    </div>
                  );
